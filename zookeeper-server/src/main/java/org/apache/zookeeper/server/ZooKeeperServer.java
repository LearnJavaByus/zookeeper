 ...
 *
 * 所有服务器的父类，其请求处理链为PrepRequestProcessor -> SyncRequestProcessor -> FinalRequestProcessor。
 */
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
     ...
     *
     *  初始化服务器统计数据、事务日志工厂、心跳时间、会话时间（最短超时时间和最长超时时间）。
     */
     ...
     *
     *  用于加载数据，其首先会判断内存库是否已经加载设置zxid，之后会调用killSession函数删除过期的会话，
     ...
     * and FinalRP.
     *
     * ChangeRecord数据结构是用于方便PrepRequestProcessor和FinalRequestProcessor之间进行信息共享，其包含了一个拷贝方法duplicate，
     * 用于返回属性相同的ChangeRecord实例。
     */
    ...
    public void processConnectRequest(ServerCnxn cnxn, ByteBuffer incomingBuffer) throws IOException {
        ...
        if (!readOnly && this instanceof ReadOnlyZooKeeperServer) { // 为只读模式但是该服务器是只读服务器，抛出异常
        ...
        // session is setup  // 不接收任何packet，直到会话创建成功
        cnxn.disableRecv();
    ...
    public void processPacket(ServerCnxn cnxn, ByteBuffer incomingBuffer) throws IOException {
        ...
        RequestHeader h = new RequestHeader(); // 创建请求头
        h.deserialize(bia, "header"); // 将头反序列化为RequestHeader
        ...
        if (h.getType() == OpCode.auth) { // 需要进行认证（有密码）
            ...
            if (authReturn == KeeperException.Code.OK) { // 认证成功
                if (LOG.isDebugEnabled()) {
